...
//    pipelineFitSave(sqlContext)
  ...
  def train_test(sqlContext: SQLContext): Unit = {
    ...
    lr.setMaxIter(10).setRegParam(0.01) // basic.set params
    ...
    // basic.set params using ParamMap
  ...
  def pipelineFitSave(sqlContext: SQLContext): Unit = {
    ...
    // data
    val training = sqlContext.createDataFrame(Seq(
      (0L, "a b c d e hadoop_mr_spark.spark", 1.0),
      (1L, "b d", 0.0),
      (2L, "hadoop_mr_spark.spark f g h", 1.0),
      (3L, "hadoop_mr_spark.hadoop mapreduce", 0.0)
    )).toDF("id", "text", "label")
    ...
    val pipeline = new Pipeline().setStages(Array(tokenizer, hashingTF, logisticRegression)) // basic.set stages in pipeline

    // fit
    val model = pipeline.fit(training) // 可以ParamMap 设置参数

    // save。此处使用 pipeline save的好处：只有一次 磁盘写入。 在exd中，每个算子都会有一次磁盘写入
    model.write.overwrite().save("/atest/hadoop_mr_spark.spark/lrmodel") // after fit
    pipeline.write.overwrite().save("/atest/hadoop_mr_spark.spark/unfit-lrmodel") // unfit pipeline
  ...
  def pipelineLoadTransform(sqlContext: SQLContext): Unit = {
    val pipeline: Pipeline   = Pipeline.load("/atest/hadoop_mr_spark.spark/unfit-lrmodel")
    val model: PipelineModel = PipelineModel.load("/atest/hadoop_mr_spark.spark/lrmodel")

    val test = sqlContext.createDataFrame(Seq(
      (4L, "hadoop_mr_spark.spark i j k"),
      (5L, "l m n"),
      (6L, "hadoop_mr_spark.spark hadoop_mr_spark.hadoop hadoop_mr_spark.spark"),
      (7L, "apache hadoop_mr_spark.hadoop")
    )).toDF("id", "text")
